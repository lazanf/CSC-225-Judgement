...
public class InGameMenu {
     ...
     ******************************************************************/
    public InGameMenu(ImageEntity background) {
        _background = background;
        SCREENWIDTH = axohEngine2.Judgement.getScreenwidth();
        SCREENHEIGHT = axohEngine2.Judgement.getScreenheight();
    ...
    public void countItems() {
        ...
        for (Item item : items) { //Add items to be counted here
            String name = item.getName();
            if (name.equals("Potion")) {
                counts[0]++;
                potions++;
            }
            if (name.equals("Mega Potion")) {
                counts[1]++;
                mpotions++;
            }
        }
    ...
    private void setItems() {
        ...
        totalEquipment = 0;
        for (Item item : items) {
            if (Objects.equals(item.getName(), "Potion")) {
                shownItems[0] = item;
            }
            if (Objects.equals(item.getName(), "Mega Potion")) {
                shownItems[1] = item;
            }
        }

        for (Item aShownEquipment : shownEquipment) {
            if (aShownEquipment == null) continue;
            totalEquipment++;
        }
        for (Item shownItem : shownItems) {
            if (shownItem == null) continue;
            totalItems++;
        }
    }

    //Reference blue code block above methods
    public void render(JPanel frame, Graphics2D g2d, int inX, int inY) {
    	SCREENWIDTH = frame.getWidth();
    	SCREENHEIGHT = frame.getHeight();
    	
        g2d.drawImage(_background.getImage(), 0, 0, SCREENWIDTH, SCREENHEIGHT, frame);
        g2d.setColor(Color.WHITE);
        g2d.setFont(new Font("Arial", Font.PLAIN, SCREENWIDTH/25));
        int x = SCREENWIDTH / 7;
        int y = (int) (SCREENHEIGHT / 5.5);
        g2d.drawString("Items", x, y);
        g2d.drawString("Equipment", x - 50, (int) (y*1.7));
        g2d.drawString("Magic", x, (int) (y*2.3));
    ...
    //Reference blue code block above methods
    public void renderItems(JPanel frame, Graphics2D g2d, Item[] array, LinkedList<Item> list) {
        ...
        for (int i = start; i < start + 4; i++) {
            ...
            g2d.setColor(Color.YELLOW);
            if (!list.isEmpty())
                g2d.drawLine(SCREENWIDTH / 3 + 120, 510 + sectionLoc * 110, 670 + array[i].getName().length() * 70 + 200, 510 + sectionLoc * 110);
            g2d.setColor(Color.WHITE);
            g2d.setFont(new Font("Arial", Font.PLAIN, 70));
            g2d.drawString(array[i].getName(), SCREENWIDTH / 3 + 120, 500 + y * 110);
            g2d.drawString(" x " + Integer.toString(counts[i]), SCREENWIDTH / 3 + (array[i].getName().length() * 70), 500 + y * 110);
    ...
    public void useItem() {
        for (int i = 0; i < items.size(); i++) {
            if (Objects.equals(shownItems[itemLocation + sectionLoc].getName(), items.get(i).getName())) {

                int checkItem = items.get(i).checkItem();
                if (checkItem == 1) ; //TODO: What to do with key items?
                ...
                if (checkItem == 3) ; //TODO: Attack Item
                items.remove(i);
                countItems();
                setItems();
                break;
    ...
    public int checkCount() {
        return counts[itemLocation + sectionLoc];
    }
    ...
    public int magicMinus() {
        return magic -= 1;
    }

    public int magicPlus() {
        return magic += 1;
    }
